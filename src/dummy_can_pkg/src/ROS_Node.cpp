...
ROS_Node::ROS_Node(/* args */)
{
   ...
   dummy_can_interface = new CAN_Interface((char *)this->_USB_PORT.c_str());
...
void ROS_Node::update()
{
   ...
   {
      dummy_can_interface->getFeedback(_can_feedback);
   ...
   // {
   //    dummy_can_interface->sendCmdVel(this->_velocity, this->_steering);
   //    dummy_can_interface->sendEmergencyBrake(this->_emergency_brake);
   //    dummy_can_interface->sendDoorControl(this->_door_control);
