...
class ros_slider:
    def __init__(self):
        ...
        self.drive_speed.data = self.steering_angle.data = 0
    ...
    def update_drive_value(self,value):
        drive_label.config(text=f"Drive: {value}m/s")

        self.drive_speed.data = int(value) 
        
        print("drive: %f  -  steer: %f " % (self.drive_speed.data,self.steering_angle.data))
    ...
    def update_steering_value(self,value):
        ...
        steering_label.config(text=f"Steering: {value} degrees")

        self.steering_angle.data = int(value)
...
if __name__ == '__main__':
    ...
    drive_label.pack()
    drive_slider = tk.Scale(window, from_=100, to=-100, orient="vertical", command=slider.update_drive_value,length=300)
    drive_slider.set(0.0)  # Set the default value to 50
    drive_slider.pack()

    # Create steering slider
    steering_label = tk.Label(window, text="Steering: 0%", font=("Arial", 16))
    steering_label.pack()
    steering_slider = tk.Scale(window, from_=16, to=-16, orient="horizontal", command=slider.update_steering_value,length=300)
    steering_slider.set(0)  # Set the default value to 50
